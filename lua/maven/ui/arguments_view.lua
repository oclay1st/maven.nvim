...
function ArgumentView:_load_options_nodes()
  options = MavenConfig.options.default_arguments_view.arguments
  local options_nodes = {}
    ...
    self._options_tree = Tree({
      ...
      prepare_node = function(node)
        ...
        if node.enabled then
          ...
          line:append(MavenConfig.options.icons.maven, highlights.ERROR)
        end
...
function ArgumentView:_create_input_component()
  self._input_component = Input({
    ...
    border = {
      text = {
        top = ' Set Default Maven Arguments ',
        top_align = 'center',
      },
      style = MavenConfig.options.default_arguments_view.input_win.border.style,
      padding = MavenConfig.options.default_arguments_view.input_win.border.padding
        or { 0, 0, 0, 0 },
    },
  }, {
...
function ArgumentView:_create_options_component()
  self._options_component = Popup(vim.tbl_deep_extend('force', self._default_opts, {
    ...
    border = {
      style = MavenConfig.options.default_arguments_view.options_win.border.style,
      padding = MavenConfig.options.default_arguments_view.options_win.border.padding
        or { 0, 0, 0, 0 },
    },
  }))
  ...
  self._options_component:map('n', '<enter>', function()
    ...
    vim.schedule(function()
      for i = 1, #MavenConfig.options.default_arguments_view.arguments do
        if
          MavenConfig.options.default_arguments_view.arguments[i].arg == current_node.arg
          and MavenConfig.options.default_arguments_view.arguments[i].value == current_node.value
        then
          if MavenConfig.options.default_arguments_view.arguments[i].enabled then
            MavenConfig.options.default_arguments_view.arguments[i].enabled = false
          else
            MavenConfig.options.default_arguments_view.arguments[i].enabled = true
          end
        end
      end
      self:_create_options_tree_list()
  ...
  self._layout = Layout(
    {
      ...
      relative = 'editor',
      position = '50%',
      size = MavenConfig.options.default_arguments_view.size,
    },
    Layout.Box({
